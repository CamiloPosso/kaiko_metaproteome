...
def spectra_header(file_handle, location):
  ...
  ## Changed the regex to allow for odd mgf files with  
  ## spaces in each line after the intensities. So, removed the $
...
def read_random_stack_for_multiple_files(data_format, file_index, spectra_locations, stack_size):
  ...
  random_idx = random.sample(range(file_index[1][-1]), min(stack_size, file_index[1][-1]))
  # random_locations = random.sample(spectra_locations,
  #                                  min(stack_size, len(spectra_locations)))
  random_locations = []
...
def read_random_stack_for_multiple_pickles(all_spectra, all_sequence, num_spectra, stack_size):
  ...
  random_idx = random.sample(range(num_spectra[-1]), min(stack_size, num_spectra[-1]))
...
def decode_beam_search_01(sess,
  ...
  for spectrum_id in range(init_size):
